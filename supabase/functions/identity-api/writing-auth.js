...
export const WRITING_PATHS = new Set(['/writing-proofs/issue','/writing-proofs/redeem','/writing-grants/check','/writing-grants/revoke','/sessions/logout']);
const statuses = { AUTH_REQUIRED:401, SESSION_EXPIRED:401, SESSION_REVOKED:401, FORBIDDEN:403, TARGET_MISMATCH:403, PROOF_USED:409, BAD_REQUEST:400 };
...
export async function writingSessionActive(db, claims) {
...
export async function handleWritingAuth(req,path,{db,secret}) {
  ...
  if(path==='/writing-proofs/issue') {
    ...
    const p=await writingAction(db,'issue',{session_id:c.central_session_id,member_id:c.sub,session_version:c.session_version,site_id:body.target_site_id,code_challenge:body.code_challenge,return_path:returnPath});
    const now=Math.floor(Date.now()/1000), exp=Math.floor(Date.parse(p.expires_at)/1000);
    const proof=await signToken({kind:'writing_proof',jti:p.id,sub:c.sub,central_session_id:c.central_session_id,session_version:c.session_version,aud:body.target_site_id,code_challenge:body.code_challenge,iat:now,exp},secret);
  ...
  const result=await writingAction(db,'redeem',{proof_id:c.jti,session_id:c.central_session_id,member_id:c.sub,session_version:c.session_version,site_id:body.site_id,code_challenge:challenge,grant_hash:await sha256(rawGrant)});
